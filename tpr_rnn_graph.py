...
with tf.variable_scope("variables"): # Note, the MLP weights are created in the MLP function.
  ...
  tt_tpr_init = [t3f.to_tt_tensor(tf.zeros((c.entity_size, c.role_size, c.entity_size))).tt_cores for i in range(_batch_size)]
  print('prezipped shape:', len(tt_tpr_init))

  # we use a cell and dynamic_rnn instead of a tf.while_loop due to its dynamic squence_length capability
  loopCell = LoopCell(tt_tpr_init)
...
with tf.variable_scope("model"):
  with tf.variable_scope("update_module"):
    ...
    def body(inputs, tt_cores_batch):
      TPR = tf.stack([t3f.full(t3f.TensorTrain(tt_cores)) for tt_cores in tt_cores_batch])
      ...
      u_tpr = tf.unstack(TPR)
      u_delta = tf.unstack(delta_F)

      return [[t3f.to_tt_tensor(d).tt_cores for d in u_delta]], [t3f.to_tt_tensor(t).tt_cores for t in u_tpr]
    ...
    _, tt_tpr_cores = tf.nn.dynamic_rnn(loopCell, inputs, initial_state=tt_tpr_init, sequence_length=story_length)

    TPR = tf.stack([t3f.full(t3f.TensorTrain(tt_cores)) for tt_cores in tt_tpr_cores])
    print('unziped shape:', TPR.shape)
...
def train(steps=1000000, bs=128, terminal_log_every=50, validate_every=200):
  ...
  with tf.contrib.tfprof.ProfileContext('./abounasm/profiler', trace_steps=[], dump_steps=[]) as pctx:
